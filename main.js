...
const App = () => {
    ...
    const tiles = [
        ...
        { id: 'M10', name: 'ARCHITECTURE', icon: 'fa-microchip', color: 'border-slate-400' },
        { id: 'M11', name: 'SENSORS (IoT)', icon: 'fa-satellite-dish', color: 'border-pink-600' },
    ...
    if (!isAuthenticated) {
        return (
            <div className="h-screen flex items-center justify-center p-6 bg-slate-950">
                <form onSubmit={handleLogin} className="glass p-10 rounded-[3rem] border border-slate-800 w-full max-sm text-center animate-fade shadow-2xl">
            ...
            {view === 'HOME' ? (
                <div className="space-y-6 animate-fade">
                    <div className="flex justify-between items-center px-4">
                        <div className="text-left font-bold italic text-white">
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Director Mode</span>
                            <p className="brand text-sm uppercase tracking-tighter">M. SYKINIOTIS</p>
                        </div>
                ...
                <div className="animate-fade h-full">
                    ...
                    {view === 'M9' && window.M9_Operations && <window.M9_Operations setView={setView} />}
                    {view === 'M12' && window.M12_StaffHub && <window.M12_StaffHub setView={setView} />}
                    
                    {['M1','M6','M7','M8','M10','M11'].includes(view) && (
